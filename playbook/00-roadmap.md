# Roadmap type — de l'idée au lancement

Template à copier dans `apps/<drop>/project/roadmap.md` et à dater. Chaque phase a une
**sortie** (deliverable) claire et un **responsable**. Adapter selon l'ampleur du drop.

---

## Phase 0 — Cadrage  ·  *sortie : brief validé*
- [ ] Brief rempli (`01-brief.md`) : vision, cible, produit, positionnement
- [ ] Modèle économique : prix, marge, coûts (fab, expédition, com)
- [ ] Parcours d'achat décidé : `configurator` / `simple` / `quote`
- [ ] Thème / direction artistique choisi (→ `@ras/themes` ou nouveau thème)
- [ ] Échéance de lancement fixée

## Phase 1 — Identité & design  ·  *sortie : maquettes validées*
- [ ] Kit de marque (`02-brand-kit.md`) : nom, logo, couleurs, typo, ton
- [ ] Thème déclaré dans `@ras/themes` (tokens)
- [ ] Maquettes générées (deck PDF via `design/mockups`) et validées
- [ ] Validation artiste / partenaire si collab

## Phase 2 — Build  ·  *sortie : site fonctionnel en préprod*
- [ ] App créée dans `apps/<drop>` (réutilise `@ras/core`, `@ras/ui`, `@ras/themes`)
- [ ] `drop.config.ts` rempli (brand, catalog, content, flow, integrations)
- [ ] Pages clés en place + responsive
- [ ] Intégrations branchées (paiement, leads, IA) en mode test

## Phase 3 — Contenu  ·  *sortie : contenu définitif intégré*
- [ ] Photos produit / visuels artiste
- [ ] Copy finale (hero, à propos, FAQ, produits)
- [ ] Avis / preuve sociale
- [ ] SEO de base (titres, métas, OG, sitemap)

## Phase 4 — Pré-lancement  ·  *sortie : prêt à ouvrir*
- [ ] Checklist mise en ligne (`04-launch-checklist.md`) déroulée
- [ ] Checklist légale (`05-legal-checklist.md`) OK
- [ ] Checklist marketing pré-lancement (`03-marketing-checklist.md`)
- [ ] Tests : commande de bout en bout, paiement réel, email de lead

## Phase 5 — Lancement (Jour J)  ·  *sortie : ouvert au public*
- [ ] Passage des intégrations en **production** (clés live)
- [ ] Annonce (réseaux, emailing, presse/influence)
- [ ] Monitoring actif (erreurs, paiements, trafic)

## Phase 6 — Post-lancement  ·  *sortie : itérations*
- [ ] Suivi métriques (trafic, conversion, panier moyen)
- [ ] Relances / retargeting / UGC
- [ ] Rétro : ce qui remonte dans le cœur `@ras/*` pour les prochains drops

---

### Jalons (à dater)
| Jalon | Date cible | Statut |
|---|---|---|
| Brief validé | | |
| Maquettes validées | | |
| Site en préprod | | |
| Go-live | | |
